...
library(rpart)
library(caret)
GermanCredit <- readxl::read_xls(file.choose())
colnames(GermanCredit)[8] <- "RADIO_TV"
colnames(GermanCredit)[18] <- "CO_APPLICANT"
attach(GermanCredit)
set.seed(1234)
ind <- sample(2, nrow(GermanCredit), replace=TRUE, prob=c(0.6, 0.4))
trainGCData <- GermanCredit[ind==1,]
testGCData <- GermanCredit[ind==2,]
myformula = RESPONSE ~ CHK_ACCT+DURATION + HISTORY + NEW_CAR+ USED_CAR+ FURNITURE+ RADIO_TV + EDUCATION+RETRAINING+AMOUNT+SAV_ACCT+EMPLOYMENT+INSTALL_RATE+MALE_DIV+MALE_SINGLE+MALE_MAR_or_WID+CO_APPLICANT+GUARANTOR+PRESENT_RESIDENT+REAL_ESTATE+PROP_UNKN_NONE+AGE+OTHER_INSTALL+RENT++OWN_RES+NUM_CREDITS+JOB+NUM_DEPENDENTS+TELEPHONE+FOREIGN

#Decision Tree without considering the cost of misclassification
set.seed(1234)
DecTree1 <- rpart(myformula,data = trainGCData,method="class")

#Decision Tree considering the cost of misclassification
DecTree2 <- rpart(myformula,data = trainGCData,method="class",parms=list(loss=matrix(c(0,5,1,0), byrow=TRUE, nrow=2)))

#Predicting the response of test data
pred1 <- predict(DecTree1,testGCData,type = "class")
pred2 <- predict(DecTree2,testGCData,type ="class")

#creating a confusion matrix
attributes(confusionMatrix(pred1,testGCData$RESPONSE)$table)
confusionMatrix(pred1,testGCData$RESPONSE)
table(pred1,testGCData$RESPONSE)
#pred1   0   1
#0      46  35
#1      95 230
confusionMatrix(pred2,testGCData$RESPONSE)
table(pred2,testGCData$RESPONSE)
#pred2   0   1
#0      107  94
#1       34 171
#Comparing these 2 confusion matrix, we can clearly see that the decision tree now considers the misclassification cost
#and adjusts accordingly to minimize the loss.
#Loss due to false postive and false negative in the model WITHOUT incorporating the oppotunity cost matrix equals
#(95x500)+(35x100)=51000
#Loss due to false postive and false negative in the model WITH incorporating the oppotunity cost matrix equals
#(34x500)+(94x100)=26400
#To conclude, the second model, that is the one which is aware of the mis-classification cost will save a lot from mis-classifying
#a bad credit risk applicant as a good credit risk applicant while maintaing the accurancy of the classification.
...
for (i in seq(from=1, to=15)){
  set.seed(1234)
  randForTic_2 <- randomForest(CARAVAN~.,data = tic0,mtry=i,sampsize=c(400,348),proximity =TRUE,importance=TRUE)
